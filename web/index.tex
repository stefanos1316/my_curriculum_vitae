...
\item
  ...
  \item
    ...
    {[}\useURL[url13][GKLS18.pdf][][Paper]\from[url13]{]}
    {[}\useURL[url14][GKLS18.bib][][BibTex]\from[url14]{]}
  ...
  \item
    ...
    {[}\useURL[url18][GKS17.bib][][BibTex]\from[url18]{]}
    {[}\useURL[url19][https://www.slideshare.net/GeorgiouStefanos/programming-languages-energy-consumption-an-empirical-study][][Presentation]\from[url19]{]}
...
\item
  ...
  \item
    ...
    2019. {[}\useURL[url21][GS19.pdf][][Preprint]\from[url21]{]}
    {[}\useURL[url22][GS19.bib][][BibTex]\from[url22]{]}
  \item
    ...
    {[}\useURL[url23][GRS19.pdf][][Preprint]\from[url23]{]}
    {[}\useURL[url24][GRS19.bib][][BibTex]\from[url24]{]}
  \stopitemize
\stopitemize
...
\startdescription{Tutorials}
  ...
  Greece.
  \useURL[url25][https://aueb-balab.github.io/courses/tools/ci_cd_with_gitlab-p.html\#/][][Presentation]\from[url25]
  ...
  \useURL[url26][https://aueb-balab.github.io/courses/tools/travis_ci_cv_template-p.html\#/][][Presentation]\from[url26]
\stopdescription
...
\startdescription{Research}
  ...
  \useURL[url27][perccom_summer_school_2018.pdf][][Proof]\from[url27]
\stopdescription
...
{[}\useURL[url28][https://github.com/AUEB-BALab/courses][][Repository]\from[url28]{]}

Gave Java lectures and tutorials for the 1st and 2nd Coding Boot Camp in
Athens, Greece (Oct.~2017 - May 2018)
{[}\useURL[url29][https://github.com/codeandwork/courses][][Repository]\from[url29]{]}
{[}\useURL[url30][boot_camp_athens_2016_2017.pdf][][Proof]\from[url30]{]}

\subsection[title={Organization
Services},reference={organization-services}]

{\em Program Committee} " ISEC 2022 for the Doctoral Symposium :
ESEC/FSE 2021 for the Student Research Competition : ASE 2020 for the
...
\startdescription{Sep 2020-current}
  {\bf Software Developer} \useURL[url31][https://gfoss.eu/][][Greek
  Free, Open Source Software]\from[url31]

  Developing a privacy-preserving epidemic dosimeter based on contact
  tracing. \useURL[url32][gfoss_2020.pdf][][Proof]\from[url32]
  \useURL[url33][https://github.com/eellak/epidose.git][][Repository]\from[url33]

  \startdescription{{\em Responsibilities}}
    Developing
    \useURL[url34][https://www.python.org/][][Python]\from[url34],
    \useURL[url35][https://www.gnu.org/software/bash/][][Shell]\from[url35],
    and \useURL[url36][https://www.ansible.com/][][Ansible]\from[url36]
...
\startdescription{Jan 2019-Apr 2020}
  ...
  \useURL[url37][https://www.allcancode.com/][][AllCanCode Inc.~Greek
  Branch]\from[url37]

  Supported the product development that facilitates fast web-sites
  development (by using
  \useURL[url38][https://developers.google.com/blockly][][Blockly]\from[url38])
  on Desktop that can be exported in smart-phones as well.

  \startdescription{{\em Responsibilities}}
    Development of API end-points in JavaScript
    (\useURL[url39][https://nodejs.org/en/][][Node.JS]\from[url39]) and
    \useURL[url40][https://www.mongodb.com/][][MongoDB]\from[url40] for
    ...
    \useURL[url41][https://mochajs.org/][][Mocha]\from[url41] and
    \useURL[url42][https://www.cypress.io/][][Cypress]\from[url42]
    ...
    \useURL[url43][https://cloud.google.com/appengine/docs/flexible][][Flex
    environment]\from[url43]

    Migration of monolithic product to micro-services using
    \useURL[url44][https://firebase.google.com/docs/hosting][][Firebase
    hosting]\from[url44],
    \useURL[url45][https://firebase.google.com/docs/functions][][Cloud
    Functions]\from[url45], and
    \useURL[url46][https://cloud.google.com/run][][Cloud
    Run]\from[url46]

    Integration of
    \useURL[url47][https://www.bitrise.io/][][Bitrise]\from[url47]
    ...
    \useURL[url48][https://cordova.apache.org/][][Cordova]\from[url48]
    wrapper
  \stopdescription
\stopdescription
...
\startdescription{Jan 2016-2019}
  ...
  \useURL[url49][https://portal.singularlogic.eu/en][][Singular Logic
  S.A.]\from[url49]

  Proposals writing for the European Projects Department.
  \useURL[url50][singular_logic_2016-2019.pdf][][Proof]\from[url50]
...
\startdescription{{\em Back-End}}
  ...
  MongoDB (\useURL[url51][mongodb_m001,pdf][][Basics]\from[url51],
  \useURL[url52][mongodb_m121.pdf][][Aggregations]\from[url52],
  \useURL[url53][mongodb_m220.pdf][][JavaScript]\from[url53], and
  \useURL[url54][mongodb_m201.pdf][][Performance]\from[url54])
\stopdescription
...
\startdescription{{\em Projects}}
  ...
  tracing
  \useURL[url55][https://github.com/eellak/epidose][][Repository]\from[url55]

  Exam Questionnaire Scanner:
  \useURL[url56][https://github.com/AntonisGkortzis/ExamQuestionnaireScanner][][Repository]\from[url56]

  Measuring Energy Consumption:
  \useURL[url57][https://github.com/stefanos1316/SEMTs_Comparisson][][Software]\from[url57]
  and
  \useURL[url58][https://stefanos1316.github.io/courses/tools/measuring_energy_consumption_direct_approach-p.html\#/][][Hardware]\from[url58]
  tools

  Programming II website:
  \useURL[url59][https://github.com/stefanos1316/courses][][Repository]\from[url59]

  Rosetta Code Experiement:
  \useURL[url60][https://github.com/stefanos1316/Rosetta_Code_Data_Set][][Repository]\from[url60]

  Validate Links:
  \useURL[url61][https://github.com/stefanos1316/validateLinks][][Repository]\from[url61]
\stopdescription

\startdescription{{\em Certificates}}
  Unix Tools: \useURL[url62][unix_tools_edx.pdf][][Data, Software, and
  Production Engineering]\from[url62]
\stopdescription

\startdescription{{\em Course Completion}}
  MongoDB: \useURL[url63][mongodb_m001.pdf][][Basics]\from[url63],
  \useURL[url64][mongodb_m121.pdf][][Aggregation]\from[url64],
  \useURL[url65][mongodb_m220.pdf][][JavaScript]\from[url65],
  \useURL[url66][mongodb_m201.pdf][][Performance]\from[url66]
\stopdescription
